...
img_feature_root_path = "../data/feature/swag"
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    ic(all_input_ids.shape)
    sampled_data_end = int(len(all_label)*args.training_percentage/100) if not evaluate else len(all_label)
    ic(sampled_data_end)
    if args.unifylangvis:
        img_features = torch.load(os.path.join(img_feature_root_path, "swag" + ("_dev" if evaluate else "_train") + ".pt"))    
        ic(img_features[0][:sampled_data_end].shape)
        dataset = TensorDataset(all_input_ids[:sampled_data_end], all_input_mask[:sampled_data_end], all_segment_ids[:sampled_data_end], all_label[:sampled_data_end], img_features[0][:sampled_data_end], img_features[1][:sampled_data_end])
    else:
        dataset = TensorDataset(all_input_ids[:sampled_data_end], all_input_mask[:sampled_data_end], all_segment_ids[:sampled_data_end], all_label[:sampled_data_end])
...
def train(args, train_dataset, model, tokenizer, model_langvis=None):
    ...
    if model_langvis and global_epoch <= args.num_langvis_train_epochs:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if model_langvis and global_epoch <= args.num_langvis_train_epochs: model_langvis.train()
            else: model.train()
            ...
            if model_langvis:
                    unifylangvis_logits, unifylangvis_loss_item = model_langvis(torch.tensor(batch[4]).cuda(args.device), torch.tensor(batch[5]).cuda(args.device), label=batch[3])
                    loss += unifylangvis_loss_item
...
def evaluate(args, model, tokenizer, prefix="", model_langvis=None):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            if model_langvis:
                unifylangvis_logits, unifylangvis_loss_item = model_langvis(torch.tensor(batch[4]).cuda(args.device), torch.tensor(batch[5]).cuda(args.device), label=batch[3])
                logits += unifylangvis_logits
                eval_loss += unifylangvis_loss_item
...
def main():
    ...
    parser.add_argument("--unifylangvis", action="store_true", help="use visual bilinear model.")
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            # Evaluate
            result = evaluate(args, model, tokenizer, prefix=global_step, model_langvis=model_langvis if args.unifylangvis else None)
